...
export default function AdOrder() {
    ...
    const [activeOrderId, setActiveOrderId] = useState(mockOrders[0]?.id || null);
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const [isFilterOpen, setIsFilterOpen] = useState(false);
    const [orderTracking, setOrderTracking] = useState({});
    const [assignments, setAssignments] = useState(() => {
        const init = {};
        mockOrders.forEach(o => { if (o.assignedTailor || o.tailor) init[o.id] = 'EMP-001'; });
        return init;
    });

    const filteredOrders = useMemo(() => {
        let result = mockOrders.filter(o =>
    ...
    const counts = useMemo(() => {
        ...
        mockOrders.forEach(o => { c.All++; const s = getDerivedStatus(o); if (c[s] !== undefined) c[s]++; });
        return c;
    }, []);

    const activeOrder = useMemo(() => mockOrders.find(o => o.id === activeOrderId), [activeOrderId]);
    ...
    const assignedEmployee = useMemo(() => {
        ...
        const empId = assignments[activeOrder.id];
        return empId ? EMPLOYEE_POOL.find(e => e.id === empId) : null;
    }, [activeOrder, assignments]);
    const earningsPreview = useMemo(() => {
        if (!activeOrder) return null;
        return computeOrderEarnings(activeOrder, assignments[activeOrder.id]);
    }, [activeOrder, assignments]);

    const handleStepClick = (orderId, stepIndex) => setOrderTracking(prev => ({ ...prev, [orderId]: stepIndex }));
    const handleAssign = (orderId, empId) => setAssignments(prev => ({ ...prev, [orderId]: empId }));
                    ...
                    <div className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-3 custom-scrollbar">
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map(order => {
                                ...
                                const listAssignee = assignments[order.id] ? EMPLOYEE_POOL.find(e => e.id === assignments[order.id]) : null;
                                return (
                                    <div key={order.id} onClick={() => setActiveOrderId(order.id)}
                                        ...
                                        <div className="p-4 pl-5">
                                            <div className="flex items-start justify-between mb-2">
                                                <span className="text-[11px] font-bold text-gray-500 tracking-wider">{order.id}</span>
                                            ...
                                            <div className="flex items-center gap-2 mb-3">
                                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${typeConf.color}`}>{order.serviceType}</span>
                                                <span className="text-xs font-medium text-gray-500 truncate max-w-[160px]" title={order.item}>{order.item}</span>
                                            </div>
                                            {listAssignee && (
                        ...
                        <div className="flex flex-col h-full">
                            <div className="p-5 lg:px-8 lg:py-6 border-b border-gray-50 shrink-0">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 mt-4 opacity-90">
                                    ...
                                    <div className="flex items-center gap-2 text-sm font-semibold text-gray-600"><Phone size={16} className="text-gray-400" />{activeOrder.invoice?.billTo?.phone || 'N/A'}</div>
                                </div>
                            </div>
                                                        ...
                                                        <div key={idx} className="relative flex items-start gap-4 cursor-pointer group" onClick={() => handleStepClick(activeOrder.id, idx)}>
                                                            ...
                                                            <div className="flex-1 -mt-1 pb-4">
                                                                ...
                                                                {isCurrent && !isCompleted && idx < activeOrderSteps.length && (
                                                                    <button onClick={e => { e.stopPropagation(); handleStepClick(activeOrder.id, idx + 1); }} className="mt-3 bg-blue-600 text-white hover:bg-blue-700 px-4 py-2 rounded-xl text-xs font-bold transition-colors shadow-sm w-full sm:w-auto border-none cursor-pointer">
                                                                        {getNextActionLabel(idx, activeOrderSteps)}
                                                                    </button>
                                                                )}
                                                            </div>
                                                        </div>
                                        ...
                                        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
                                            <div className="bg-blue-50/50 p-5 border-b border-gray-50">
                                                ...
                                                <div className="space-y-4">
                                                    <div className="flex justify-between items-center text-sm"><span className="font-semibold text-gray-500">Drop Date</span><span className="font-bold text-gray-900">{activeOrder.date}</span></div>
                                                    <div className="flex justify-between items-center text-sm"><span className="font-semibold text-gray-500">Due Date</span><span className={`font-bold ${isOverdue(activeOrder.invoice?.dueDate || activeOrder.estimatedCompletion) ? 'text-red-500' : 'text-gray-900'}`}>{activeOrder.invoice?.dueDate || activeOrder.estimatedCompletion}</span></div>
