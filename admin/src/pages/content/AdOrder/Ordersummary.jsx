...
export default function OrderSummary({ activeOrder, assignedEmployee }) {
    const totalPrice = Number(
        activeOrder.invoice?.total || 
        activeOrder.totalPrice || 
        activeOrder.invoice?.items?.reduce((s, i) => s + (Number(i.unitPrice) || 0) * (Number(i.qty) || 1) + (Number(i.addOnPrice) || 0), 0) || 0
    );
            ...
            <div className="p-4 bg-white">
                <div className="space-y-3 mb-4">
                    {activeOrder.invoice?.items?.map((item, idx) => {
                        const itemTotal = (Number(item.qty) || 1) * (Number(item.unitPrice) || 0) + (Number(item.addOnPrice) || 0);
                        return (
