...
import { Clock, Check, User, Lock, CalendarClock, ShieldCheck } from 'lucide-react';
...
export default function ProductionTimeline({ activeOrderSteps, currentStepIdx, onStepClick, orderId, isForApproval, hasSchedule }) {
    ...
    const lockReason = isForApproval
        ? {
            ...
            desc: 'The admin needs to approve this order before production can begin.',
          }
        : !hasSchedule
        ? {
            icon: CalendarClock,
            title: 'No Schedule Set',
            desc: 'A pickup date and time must be set before proceeding with production.',
          }
        : null;
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm relative">
            {isLocked && lockReason && (
                <div className="absolute inset-0 bg-white/80 rounded-2xl flex items-center justify-center z-20 backdrop-blur-[2px]">
                ...
                <div className="relative pl-4 space-y-6">
                    ...
                    {activeOrderSteps.map((step, idx) => {
                        ...
                        return (
                            <div
                                key={idx}
                                className={`relative flex items-start gap-4 ${isLocked ? 'cursor-not-allowed' : 'cursor-pointer group'}`}
                                onClick={() => !isLocked && onStepClick(orderId, idx)}
                            >
