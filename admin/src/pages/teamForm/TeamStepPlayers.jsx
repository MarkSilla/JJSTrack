...
import { UserPlus, Edit2, Trash2, Check, X, Shirt, Ruler, Maximize2, Minimize2 } from 'lucide-react'
...
const SizeReferencePanel = ({ onClose }) => {
    ...
    const charts = [
        { label: 'Jersey / Top', src: image.SCTOP, alt: 'Jersey and top size chart' },
        { label: 'Shorts / Bottom', src: image.SCBOT, alt: 'Shorts and bottom size chart' },
    ]
...
const getAddOnText = (player) => {
    const labels = (Array.isArray(player.addOns) ? player.addOns : [])
        .map((id) => OPTIONAL_PRODUCT_TYPES.find((p) => p.id === id)?.label)
        .filter(Boolean)
...
const getPlayerDisplayName = (player) => {
    const surname = player.surname || player.firstName || ''
    return [surname, player.nickname ? `(${player.nickname})` : ''].filter(Boolean).join(' ') || 'N/A'
}
                ...
                <tbody>
                    {players.map((pl, i) => (
                        <tr key={i} className="hover:bg-blue-50/50">
                            ...
                            <td className="border border-gray-300 px-2 py-2 font-semibold uppercase">{getPlayerDisplayName(pl)}</td>
                            ...
                            <td className="border border-gray-300 px-2 py-2">{getAddOnText(pl)}</td>
...
const TeamStepPlayers = ({ teamName, setTeamName, players, setPlayers, contact = {}, onSizeGuideChange }) => {
    ...
    const startEdit = (i) => {
        setForm({
            ...emptyPlayer,
            ...players[i],
            surname: players[i]?.surname || players[i]?.firstName || '',
    ...
    const contentWidthClass = showSizeGuide ? 'max-w-7xl' : 'max-w-4xl'
    ...
    return (
        <section>
            ...
            {/* AddEdit Player Form */}
            <div className={`${showSizeGuide ? 'max-w-7xl' : 'max-w-4xl'} mx-auto mb-6`}>
