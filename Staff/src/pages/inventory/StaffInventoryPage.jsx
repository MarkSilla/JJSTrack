            ...
            <>
              <div>
                ...
                {qty && parseInt(qty) > totalStock && (
                  <p className="text-xs text-red-500 mt-1.5 font-medium">⚠ Exceeds available stock ({totalStock} {item.unit})</p>
                )}
              </div>
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
                  ...
                  <div className="space-y-2">
                    {fifoPreview.breakdown.map((b, i) => (
                      <div key={b.batchId} className="flex items-center gap-3">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold flex-shrink-0 ${i === 0 ? "bg-amber-400 text-white" : "bg-amber-200 text-amber-700"
                          }`}>{i + 1}</div>
                  ...
                  {!fifoPreview.canFulfill && (
                    <p className="text-xs text-red-600 font-semibold mt-3 pt-3 border-t border-amber-200">
                      ⚠ Insufficient stock — still short by {fifoPreview.shortfall} {item.unit}
                    </p>
                  )}
                </div>
              )}
            ...
            <>
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                ...
                <p className="text-sm text-slate-600">
                  This will use <span className="font-bold text-blue-700">{qty} {item.unit}</span> from <span className="font-bold">{item.name}</span>
                </p>
                <p className="text-xs text-slate-400 mt-2">
                  Remaining: {totalStock - parseInt(qty)} {item.unit} · FIFO order
                </p>
              </div>
  ...
  if (history.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 text-center">
        ...
        <p className="text-sm font-semibold text-slate-500">No usage history yet</p>
        <p className="text-xs text-slate-400 mt-1">Use an item to see it here</p>
      </div>
    );
  }
...
export default function StaffInventoryPage() {
  ...
  const outOfStockCount = inventory.filter(i => getItemStock(i._id) === 0).length;
